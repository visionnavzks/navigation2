...
const PARAM_HELP_TEXTS = {
    ...
    terminal_speed: '参考路径终点的目标速度，单位 m/s。默认 0 表示到终点减速停车；调大后终点允许保留一定速度，不强制停车。',
    ...
    start_v: '目标点模式下起点的初始速度，单位 m/s。每次求解都会用这个值覆盖起点状态的速度分量。',
    ...
    w_length: '轨迹总长度代价权重。越大，优化越倾向缩短实际行驶路程（不只是时间），抑制绕远路。默认 0 表示不生效。',
