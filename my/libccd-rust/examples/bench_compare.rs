...
fn parse_filter(value: Option<String>) -> BenchFilter {
    let Some(value) = value else {
        return BenchFilter { group: None, case: None };
    };
...
fn run_bench<S1: Shape, S2: Shape>(ccd: &Ccd, algo: &Algo, obj1: &S1, obj2: &S2, cycles: u64) -> Duration {
...
fn bench_boxbox(ccd: &Ccd, algo: &Algo, cycles: u64, filter: &BenchFilter) -> Duration {
    ...
    total += bench_and_print(ccd, algo, filter, "boxbox", &mut case_num, &box1, &box2, cycles);
    total += bench_and_print(ccd, algo, filter, "boxbox", &mut case_num, &box2, &box1, cycles);
    ...
    let rot_z = quat(Vec3::Z_AXIS, PI / 4.0);
    let box1 = with_box_transform(box_shape(1.0, 1.0, 1.0), Vec3::new(-0.5, 0.0, 0.0), Some(rot_z));
    let box2 = box_shape(1.0, 1.0, 1.0);
    total += bench_and_print(ccd, algo, filter, "boxbox", &mut case_num, &box1, &box2, cycles);
    total += bench_and_print(ccd, algo, filter, "boxbox", &mut case_num, &box2, &box1, cycles);

    let rot_z = quat(Vec3::Z_AXIS, PI / 4.0);
    let box1 = with_box_transform(box_shape(1.0, 1.0, 1.0), Vec3::new(-0.5, 0.5, 0.0), Some(rot_z));
    let box2 = box_shape(1.0, 1.0, 1.0);
    total += bench_and_print(ccd, algo, filter, "boxbox", &mut case_num, &box1, &box2, cycles);
    total += bench_and_print(ccd, algo, filter, "boxbox", &mut case_num, &box2, &box1, cycles);

    let rot = quat(Vec3::new(0.0, 1.0, 1.0), PI / 4.0);
    let box1 = with_box_transform(box_shape(1.0, 1.0, 1.0), Vec3::new(-0.5, 0.1, 0.4), Some(rot));
    let box2 = box_shape(1.0, 1.0, 1.0);
    total += bench_and_print(ccd, algo, filter, "boxbox", &mut case_num, &box1, &box2, cycles);
    total += bench_and_print(ccd, algo, filter, "boxbox", &mut case_num, &box2, &box1, cycles);

    let rot = quat(Vec3::new(0.0, 1.0, 1.0), PI / 4.0)
        .multiply(quat(Vec3::new(1.0, 1.0, 1.0), PI / 4.0));
    let box1 = with_box_transform(box_shape(1.0, 1.0, 1.0), Vec3::new(-0.5, 0.1, 0.4), Some(rot));
    let box2 = box_shape(1.0, 1.0, 1.0);
    total += bench_and_print(ccd, algo, filter, "boxbox", &mut case_num, &box1, &box2, cycles);
    total += bench_and_print(ccd, algo, filter, "boxbox", &mut case_num, &box2, &box1, cycles);

    let rot = quat(Vec3::Z_AXIS, PI / 4.0)
        .multiply(quat(Vec3::X_AXIS, PI / 4.0));
    let box1 = with_box_transform(box_shape(1.0, 1.0, 1.0), Vec3::new(-1.3, 0.0, 0.0), Some(rot));
...
fn bench_cylcyl(ccd: &Ccd, algo: &Algo, cycles: u64, filter: &BenchFilter) -> Duration {
    ...
    total += bench_and_print(ccd, algo, filter, "cylcyl", &mut case_num, &cyl1, &cyl2, cycles);
    total += bench_and_print(ccd, algo, filter, "cylcyl", &mut case_num, &cyl2, &cyl1, cycles);
    ...
    let rot = quat(Vec3::new(0.0, 1.0, 1.0), PI / 4.0);
    let cyl1 = cylinder_shape(0.35, 0.5);
    let cyl2 = with_cyl_transform(cylinder_shape(0.5, 1.0), Vec3::new(-0.2, 0.7, 0.2), Some(rot));
    total += bench_and_print(ccd, algo, filter, "cylcyl", &mut case_num, &cyl1, &cyl2, cycles);
    total += bench_and_print(ccd, algo, filter, "cylcyl", &mut case_num, &cyl2, &cyl1, cycles);

    let rot = quat(Vec3::new(0.567, 1.2, 1.0), PI / 4.0);
    let cyl1 = cylinder_shape(0.35, 0.5);
    let cyl2 = with_cyl_transform(cylinder_shape(0.5, 1.0), Vec3::new(0.6, -0.7, 0.2), Some(rot));
    total += bench_and_print(ccd, algo, filter, "cylcyl", &mut case_num, &cyl1, &cyl2, cycles);
    total += bench_and_print(ccd, algo, filter, "cylcyl", &mut case_num, &cyl2, &cyl1, cycles);
...
fn bench_boxcyl(ccd: &Ccd, algo: &Algo, cycles: u64, filter: &BenchFilter) -> Duration {
    ...
    total += bench_and_print(ccd, algo, filter, "boxcyl", &mut case_num, &box_obj, &cyl, cycles);
    total += bench_and_print(ccd, algo, filter, "boxcyl", &mut case_num, &cyl, &box_obj, cycles);
