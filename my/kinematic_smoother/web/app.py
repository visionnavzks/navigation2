...
import py_kinematic_smoother as pcs  # noqa: E402
...
def _run_smoother_stage(
    ...
    try:
        ...
        smooth_error = _error_payload(exc, default_status=422, default_source="kinematic_smoother")
        smooth_message = smooth_error["message"]
        stage = _make_pipeline_stage(
            "kinematic_smoother",
            optimizer_label,
...
def plan_and_smooth():
    ...
    try:
        ...
        optimizer_type = str(req.get("optimizer_type", "kinematic_smoother")).strip().lower()
        if optimizer_type not in {"kinematic_smoother", "kinematic_smoother"}:
