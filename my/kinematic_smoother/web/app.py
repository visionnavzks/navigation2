...
def _run_ceres_smoother(params, x_ref, y_ref, theta_ref, gears,
    ...
    payload = {
        ...
        'params': {
            ...
            'max_iterations': int(params.get('ceres_max_iter',
                                             params.get('max_iterations', 500))),
            'tolerance': float(params.get('ceres_tol',
                                          params.get('tolerance', 1e-6))),
            'fix_start_kappa': bool(params.get('fix_start_kappa', True)),
            'kappa_start': float(params.get('kappa_start', 0.0)
                                 if params.get('kappa_start') is not None else 0.0),
