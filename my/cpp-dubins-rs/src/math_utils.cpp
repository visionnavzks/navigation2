#include "steering_functions_lite/math_utils.h"

namespace dubins_rs
{

    double sgn(double x)
    {
        return (x < 0.0) ? -1.0 : 1.0;
    }

    std::pair<double, double> polar(double x, double y)
    {
        double r = std::sqrt(x * x + y * y);
        double theta = std::atan2(y, x);
        return {r, theta};
    }

    double twopify(double alpha)
    {
        return alpha - TWO_PI * std::floor(alpha / TWO_PI);
    }

    double pify(double alpha)
    {
        double v = std::fmod(alpha, TWO_PI);
        if (v < -M_PI)
            v += TWO_PI;
        else if (v > M_PI)
            v -= TWO_PI;
        return v;
    }

    namespace
    {
        const double CHEBEV_A[18] = {
            0.76435138664186000189, -0.43135547547660179313, 0.43288199979726653054,
            -0.26973310338387111029, 0.08416045320876935378, -0.01546524484461381958,
            0.00187855423439822018, -0.00016264977618887547, 0.00001057397656383260,
            -0.00000053609339889243, 0.00000002181658454933, -0.00000000072901621186,
            0.00000000002037332548, -0.00000000000048344033, 0.00000000000000986533,
            -0.00000000000000017502, 0.00000000000000000272, -0.00000000000000000004};

        const double CHEBEV_B[17] = {
            0.63041404314570539241, -0.42344511405705333544, 0.37617172643343656625,
            -0.16249489154509567415, 0.03822255778633008694, -0.00564563477132190899,
            0.00057454951976897367, -0.00004287071532102004, 0.00000245120749923299,
            -0.00000011098841840868, 0.00000000408249731696, -0.00000000012449830219,
            0.00000000000320048425, -0.00000000000007032416, 0.00000000000000133638,
            -0.00000000000000002219, 0.00000000000000000032};

        const double CHEBEV_E[41] = {
            0.97462779093296822410, -0.02424701873969321371, 0.00103400906842977317,
            -0.00008052450246908016, 0.00000905962481966582, -0.00000131016996757743,
            0.00000022770820391497, -0.00000004558623552026, 0.00000001021567537083,
            -0.00000000251114508133, 0.00000000066704761275, -0.00000000018931512852,
            0.00000000005689898935, -0.00000000001798219359, 0.00000000000594162963,
            -0.00000000000204285065, 0.00000000000072797580, -0.00000000000026797428,
            0.00000000000010160694, -0.00000000000003958559, 0.00000000000001581262,
            -0.00000000000000646411, 0.00000000000000269981, -0.00000000000000115038,
            0.00000000000000049942, -0.00000000000000022064, 0.00000000000000009910,
            -0.00000000000000004520, 0.00000000000000002092, -0.00000000000000000982,
            0.00000000000000000467, -0.00000000000000000225, 0.00000000000000000110,
            -0.00000000000000000054, 0.00000000000000000027, -0.00000000000000000014,
            0.00000000000000000007, -0.00000000000000000004, 0.00000000000000000002,
            -0.00000000000000000001, 0.00000000000000000001};

        const double CHEBEV_F[35] = {
            0.99461545179407928910, -0.00524276766084297210, 0.00013325864229883909,
            -0.00000770856452642713, 0.00000070848077032045, -0.00000008812517411602,
            0.00000001359784717148, -0.00000000246858295747, 0.00000000050925789921,
            -0.00000000011653400634, 0.00000000002906578309, -0.00000000000779847361,
            0.00000000000222802542, -0.00000000000067239338, 0.00000000000021296411,
            -0.00000000000007041482, 0.00000000000002419805, -0.00000000000000861080,
            0.00000000000000316287, -0.00000000000000119596, 0.00000000000000046444,
            -0.00000000000000018485, 0.00000000000000007527, -0.00000000000000003131,
            0.00000000000000001328, -0.00000000000000000574, 0.00000000000000000252,
            -0.00000000000000000113, 0.00000000000000000051, -0.00000000000000000024,
            0.00000000000000000011, -0.00000000000000000005, 0.00000000000000000002,
            -0.00000000000000000001, 0.00000000000000000001};

        std::pair<double, double> fresnel_0_8(double x)
        {
            double quarter_x = 0.25 * x;
            double arg = 0.03125 * x * x - 1.0;
            double t0 = 1.0;
            double t1 = 0.125 * x;
            double t2 = arg;
            double t3 = quarter_x * t2 - t1;

            double a_sum = CHEBEV_A[0] * t0 + CHEBEV_A[1] * t2;
            double b_sum = CHEBEV_B[0] * t1 + CHEBEV_B[1] * t3;

            double t2n_m4 = t0;
            double t2n_m2 = t2;
            double t2n_m1 = t3;

            for (int n = 2; n < 17; ++n)
            {
                double t2n = 2.0 * arg * t2n_m2 - t2n_m4;
                double t2n_p1 = quarter_x * t2n - t2n_m1;
                a_sum += CHEBEV_A[n] * t2n;
                b_sum += CHEBEV_B[n] * t2n_p1;
                t2n_m4 = t2n_m2;
                t2n_m2 = t2n;
                t2n_m1 = t2n_p1;
            }
            double t34 = 2.0 * arg * t2n_m2 - t2n_m4;
            a_sum += CHEBEV_A[17] * t34;

            double sqrt_x = std::sqrt(x);
            double c_f = SQRT_TWO_PI_INV * sqrt_x * a_sum;
            double s_f = SQRT_TWO_PI_INV * sqrt_x * b_sum;
            return {s_f, c_f};
        }

        std::pair<double, double> fresnel_8_inf(double x)
        {
            double arg = 128.0 / (x * x) - 1.0;
            double t0 = 1.0;
            double t2 = arg;
            double e_sum = CHEBEV_E[0] * t0 + CHEBEV_E[1] * t2;
            double f_sum = CHEBEV_F[0] * t0 + CHEBEV_F[1] * t2;

            double t2n_m4 = t0;
            double t2n_m2 = t2;

            for (int n = 2; n < 35; ++n)
            {
                double t2n = 2.0 * arg * t2n_m2 - t2n_m4;
                e_sum += CHEBEV_E[n] * t2n;
                f_sum += CHEBEV_F[n] * t2n;
                t2n_m4 = t2n_m2;
                t2n_m2 = t2n;
            }
            for (int n = 35; n < 41; ++n)
            {
                double t2n = 2.0 * arg * t2n_m2 - t2n_m4;
                e_sum += CHEBEV_E[n] * t2n;
                t2n_m4 = t2n_m2;
                t2n_m2 = t2n;
            }

            double sin_x = std::sin(x);
            double cos_x = std::cos(x);
            double sqrt_x = std::sqrt(x);
            double c_f = 0.5 - SQRT_TWO_PI_INV * (e_sum * cos_x / (2.0 * x) - f_sum * sin_x) / sqrt_x;
            double s_f = 0.5 - SQRT_TWO_PI_INV * (e_sum * sin_x / (2.0 * x) + f_sum * cos_x) / sqrt_x;
            return {s_f, c_f};
        }
    } // anonymous namespace

    std::pair<double, double> fresnel(double s)
    {
        double x = HALF_PI * s * s;
        auto [s_f, c_f] = (x <= 8.0) ? fresnel_0_8(x) : fresnel_8_inf(x);
        if (s < 0.0)
        {
            s_f = -s_f;
            c_f = -c_f;
        }
        return {s_f, c_f};
    }

    void end_of_clothoid(double x_i, double y_i, double theta_i, double kappa_i,
                         double sigma, double direction, double length,
                         double& x_f, double& y_f, double& theta_f, double& kappa_f)
    {
        double sgn_sigma = sgn(sigma);
        double abs_sigma = std::fabs(sigma);
        double sqrt_sigma_inv = 1.0 / std::sqrt(abs_sigma);
        double k1 = theta_i - 0.5 * direction * kappa_i * kappa_i / sigma;
        double k2 = SQRT_PI_INV * sqrt_sigma_inv * (abs_sigma * length + sgn_sigma * kappa_i);
        double k3 = SQRT_PI_INV * sqrt_sigma_inv * sgn_sigma * kappa_i;
        double cos_k1 = std::cos(k1);
        double sin_k1 = std::sin(k1);
        auto [fresnel_s_k2, fresnel_c_k2] = fresnel(k2);
        auto [fresnel_s_k3, fresnel_c_k3] = fresnel(k3);
        x_f = x_i + SQRT_PI * sqrt_sigma_inv *
                          (direction * cos_k1 * (fresnel_c_k2 - fresnel_c_k3) -
                           sgn_sigma * sin_k1 * (fresnel_s_k2 - fresnel_s_k3));
        y_f = y_i + SQRT_PI * sqrt_sigma_inv *
                          (direction * sin_k1 * (fresnel_c_k2 - fresnel_c_k3) +
                           sgn_sigma * cos_k1 * (fresnel_s_k2 - fresnel_s_k3));
        theta_f = pify(theta_i + kappa_i * direction * length + 0.5 * sigma * direction * length * length);
        kappa_f = kappa_i + sigma * length;
    }

    void end_of_circular_arc(double x_i, double y_i, double theta_i,
                             double kappa, double direction, double length,
                             double& x_f, double& y_f, double& theta_f)
    {
        x_f = x_i + (1.0 / kappa) * (-std::sin(theta_i) + std::sin(theta_i + direction * length * kappa));
        y_f = y_i + (1.0 / kappa) * (std::cos(theta_i) - std::cos(theta_i + direction * length * kappa));
        theta_f = pify(theta_i + kappa * direction * length);
    }

    void end_of_straight_line(double x_i, double y_i, double theta,
                              double direction, double length,
                              double& x_f, double& y_f)
    {
        x_f = x_i + direction * length * std::cos(theta);
        y_f = y_i + direction * length * std::sin(theta);
    }

} // namespace dubins_rs
