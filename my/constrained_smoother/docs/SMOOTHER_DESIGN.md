...
输入输出语义已经单独整理到 `SMOOTHER_INPUT_OUTPUT.md`，避免接口约定和实现设计长期混在同一份文档里。

- 如果你要确认 `Smoother::smooth(...)` 的入参、原地输出语义、失败返回方式和 `SmoothingFailureInfo` 字段，直接看 `SMOOTHER_INPUT_OUTPUT.md`。
- 如果你要理解这些输入是如何进入“锚定、残差构建、求解、重建、后验校验”这条内部链路的，继续阅读本文后面的实现章节。
...
如果路径只有三个点且两个选项都开启，当前实现会直接拒绝该输入组合。
原因是唯一的中间点无法同时作为“第二个点”和“倒数第二个点”来严格满足两侧的几何锚定；继续求解只会把二者混成一个折中点，语义不再明确。
...
- 新增 failure reason 时只改抛错文本，不改稳定 reason / code 文档。
    - 相关改动至少要同步 `exceptions.hpp`、[Error Codes](error-codes.md)，必要时同步 README 的失败传播说明。
