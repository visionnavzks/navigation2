...
如果路径只有 3 个点，并且起终点朝向约束同时落在同一个中间点上，当前实现会直接拒绝该输入组合。
原因是唯一的中间点无法同时充当第二个点和倒数第二个点来严格满足两端锚定；继续做只会得到一个折中位置，而不是明确满足两端约束的几何结果。
...
### 12.2 `costmap` 只在启用障碍物相关项时必须提供

如果 `costmap_weight_sqrt` 和 `cusp_costmap_weight_sqrt` 都为 0，当前实现允许 `costmap = nullptr`，并会跳过 ESDF 构建、障碍物残差和障碍物后验校验。

如果启用了任一障碍物相关项，调用方仍然必须提供有效 `costmap`，因为这时求解器和校验器都会访问地图分辨率、原点和 ESDF 数据。
